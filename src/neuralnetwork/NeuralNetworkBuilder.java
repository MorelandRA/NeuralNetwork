...
public class NeuralNetworkBuilder {
	...
	public static GraphNeuralNetwork buildSampleNetwork(DataSet data, int intermediate) {
	...
	protected static void relate(Perceptron parent, Perceptron child, double weight) {
		parent.assignChild(child);
		child.assignParent(parent, weight);
	}

	// Tries to generate a neural network from a chosen save file
	public static GraphNeuralNetwork loadFrom(String file) throws IOException {
		Scanner in = null;
		try {
		...
		while (in.hasNextLine()) {
			...
			if (perceptrons.containsKey(newPerceptronID)) {
				...
				for (Perceptron child : placeholderPerceptron.getChildren()) {
					relate(newPerceptron, child, child.getParentWeightMap().get(placeholderPerceptron));
				}
	...
	static public GraphNeuralNetwork clone(GraphNeuralNetwork nn) {
		...
		while (!deque.isEmpty()) {
			...
			if (!added.contains(next)) {
				...
				perceptrons.put(newPerceptron.getPerceptronID(), newPerceptron);
				newPerceptron.setBias(next.getBias());
				for (Entry<Perceptron, Double> parentWeightPair : next.getParentWeightMap().entrySet()) {
